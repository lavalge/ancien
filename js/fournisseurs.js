...
const Fournisseurs = {
    ...
    selectedCategories: [],
    searchTerm: '',
    externalKey: 'fournisseurs_externes',
    ...
    render() {
        ...
        const external = this._getExternalFournisseursByDept(this.selectedDept);
        let fournisseurs = deptData ? deptData.fournisseurs.slice() : [];

        if (external.length > 0) {
            fournisseurs = fournisseurs.concat(external);
        }

        fournisseurs = fournisseurs.map(f => ({
            ...f,
            specialites: Array.isArray(f.specialites) ? f.specialites : []
        }));
        ...
        const allCategories = [...new Set(fournisseurs.map(f => f.categorie).filter(Boolean))].sort();
    ...
    _getExternalFournisseursByDept(code) {
        if (!code || typeof Storage === 'undefined') return [];
        const list = Storage.load(this.externalKey, []);
        const filtered = list.filter(f => (f.dept || f.departement) === code);
        const expanded = [];

        filtered.forEach(f => {
            const categories = Array.isArray(f.categories) && f.categories.length > 0
                ? f.categories
                : [f.categorie].filter(Boolean);
            ...
            categories.forEach(cat => {
                expanded.push({
                    ...
                    catalogue: f.catalogue || f.site || null,
                    delai: f.delai || null,
                    rayon: f.zones_livraison || f.rayon || null
                });
            });
        });
